...
class IssueObserver
{
    public function creating(Issue $issue)
    {
        ...
        if (!isset($issue->user_id)) {
            $issue->user_id = Auth::user()->githubUser()->id;
        }
        ...
        $tmp = app(Auth::user()->githubUser()->provider)->createOrUpdateIssue($issue);
        if (isset($tmp->id)) {
            $issue->provider_id = $tmp->id;
            $issue->number = $tmp->number;
        }

        $issue->provider = strtolower(Auth::user()->githubUser()->provider);
    ...
    public function updating($issue)
    {
        if (isset($issue->number)) {
            app(Auth::user()->githubUser()->provider)->createOrUpdateIssue($issue);
        }
        (new Status())->track('issues', $issue);
    }
